	...
	$(document).ready(function() {
		...
		try{
			...
			if(accessLevel!="" && accessLevel!=undefined){
				if (accessLevel == locationPrivilage) {
					//alert("locationPrivilage ========>>>> "+locationPrivilage);
					$("#locationIds").show();
				}
				if (accessLevel == resourcePrivilage) {
					//alert("resourcePrivilage ========>>>> "+resourcePrivilage);
					$("#resourceIds").show();
				}
			}
		}catch(e){
...
<form:form action="update-user.html" modelAttribute="adminInfoTOs" method="POST" name="addUser" id="addUser" commandName="adminInfoTOs">
	<h1>&nbsp;&nbsp;Edit User</h1>
		...
		<div class="popup-box">
			...
			<div>
				<dl>
					...
					<dd>
						<form:input path="userName" name="txtUserName" id="txtUserName"  onchange="validateUserName(${adminInfoTOs.id});"/>
						<span class="errorTxtUserName"></span>
					</dd>
					<form:hidden path="password" name="txtPassword" id="txtPassword" />
					<input type="hidden" id="txtConfirmPassword" value="${adminInfoTOs.password}" />
					<!--
					<dt>
					...
					<dd>
						<form:password path="password" name="txtPassword"
							id="txtPassword" onchange="validatePassword();" value="${adminInfoTOs.password}" />
					...
					<dd>
						<input type="password" name="txtConfirmPassword"
							id="txtConfirmPassword" value="${adminInfoTOs.password}" />
					</dd>
					-->
					<dt>
					...
					<dd class="noWidth">
						<c:forEach items="${privilageResponse.privilageList}" var="accessPrivilege">
							...
							<c:if test="${accessPrivilege.privilageName=='Provider'}">
								<form:radiobutton path="accessLevel" value="${accessPrivilege.privilageName}" 
									class="noWidth" onchange="enableResourceDropDown()" />${accessPrivilege.privilageName}
								<br />
	...
	<div class="popup-button">
			...
			&nbsp;      
			<input type="submit" class="btn noWidth" value="Update User"  id="btnUpdateUser" />
	</div>
</form:form>